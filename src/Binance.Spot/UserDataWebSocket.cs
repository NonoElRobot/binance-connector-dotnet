namespace Binance.Spot
{
    ...
    public class UserDataWebSocket : BinanceWebSocket
    {
        ...
        public UserDataWebSocket(
            string listenKey,
            IBinanceWebSocketHandler handler,
            string baseUrl = DEFAULT_USER_DATA_WEBSOCKET_BASE_URL)
        ...
        public UserDataWebSocket(string[] listenKeys, string baseUrl = DEFAULT_USER_DATA_WEBSOCKET_BASE_URL)
            : base(
                new BinanceWebSocketHandler(new ClientWebSocket()),
                baseUrl + "/stream?streams=" + string.Join("/", listenKeys))
        {
        }
